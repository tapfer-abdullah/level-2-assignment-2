import { TInventory, TProduct } from "../Product/products.interface";
...
const createOrder = async (orderData: TOrder) => {
  ...
  const response = (await productModel.findById(productId).select("inventory").lean()) as TProduct | null;
  ...
  if (response && response.inventory.inStock) {
    ...
    // if stock quantity and buy quantity are same then make the stock false and quantity = 0
      ...
      const update = await productModel.findByIdAndUpdate(productId, { $set: { "inventory.inStock": false, "inventory.quantity": 0 } }, { new: true });
    ...
    //everything is okay for creating an order
      ...
      const update = await productModel.findByIdAndUpdate(productId, { $set: { "inventory.quantity": response.inventory.quantity - orderData.quantity } }, { new: true });
      const result = await orderModel.create(orderData);
      return {
        success: true,
        message: "Order created successfully!",
